...
insert into member values ('ksseo63@naver.com', '!rudtjq0', '서경섭', '관리자', '010-9530-3135', 1, 53, '서울시 강남구 삼성로 212 6-906 06284', 'M', '1963-07-11', '자바', 'admin', 4,  '2015-09-16', '2015-09-16');
...
update board set display_format=3 where board_id='6';
...
insert into board (board_name, board_type, url, secret, read_allow, write_allow, reply, modify, remove, download, upload, nAttach, aSize, display_format, board_desc)
			values ('시 모음', '기본', 'board/list.jsp?boardId=1&p=1', 'F', 'all', 'admin', 'not', 'admin', 'admin', 'login', 'admin', 5, '10MB', 1, '시를  모아놓은 곳');
insert into board (board_name, board_type, url, secret, read_allow, write_allow, reply, modify, remove, download, upload, nAttach, aSize, display_format, board_desc)
			values ('육아 정보', '기본', 'board/list.jsp?boardId=2&p=1', 'F', 'all', 'admin', 'not', 'admin', 'admin', 'login', 'admin', 5, '10MB', 1, '육아 정보를 모아놓은 곳');
insert into board (board_name, board_type, url, secret, read_allow, write_allow, reply, modify, remove, download, upload, nAttach, aSize, display_format, board_desc)
			values ('도시농업', '기본', 'board/list.jsp?boardId=3&p=1', 'F', 'all', 'admin', 'not', 'admin', 'admin', 'login', 'admin', 5, '10MB', 1, '도시농업에 대한 정보를 공유하는 곳');
insert into board (board_name, board_type, url, secret, read_allow, write_allow, reply, modify, remove, download, upload, nAttach, aSize, display_format, board_desc)
			values ('Android', '기본', 'board/list.jsp?boardId=4&p=1', 'F', 'all', 'admin', 'not', 'admin', 'admin', 'login', 'admin', 5, '10MB', 1, 'Android 앱을 개발하기 위하여 필요한 정보를 공유하는 게시판');
insert into board (board_name, board_type, url, secret, read_allow, write_allow, reply, modify, remove, download, upload, nAttach, aSize, display_format, board_desc)
			values ('Web(자바기반)', '기본', 'board/list.jsp?boardId=5&p=1', 'F', 'all', 'admin', 'not', 'admin', 'admin', 'login', 'admin', 5, '10MB', 1, '자바기반의 Web을 개발하기 위하여 필요한 정보를 공유하는 곳');
insert into board (board_name, board_type, url, secret, read_allow, write_allow, reply, modify, remove, download, upload, nAttach, aSize, display_format, board_desc)
			values ('임베디드', '기본', 'board/list.jsp?boardId=6&p=1', 'F', 'all', 'admin', 'not', 'admin', 'admin', 'login', 'admin', 5, '10MB', 3, '임베디드 기술에 대한 정보를 공유하는 곳');
insert into board (board_name, board_type, url, secret, read_allow, write_allow, reply, modify, remove, download, upload, nAttach, aSize, display_format, board_desc)
			values ('함께 만들어요', '기본', 'board/list.jsp?boardId=7&p=1', 'F', 'all', 'login', 'not', 'admin', 'admin', 'login', 'login', 5, '10MB', 1, '회원과 운영자가 함께 정보를 만들어 가는 곳');
insert into board (board_name, board_type, url, secret, read_allow, write_allow, reply, modify, remove, download, upload, nAttach, aSize, display_format, board_desc)
			values ('공지사항', '공지사항', 'board/list.jsp?boardId=8&p=1', 'F', 'all', 'admin', 'not', 'admin', 'admin', 'login', 'admin', 5, '10MB', 3, '관리자가 회원들에게 알리는 사항을 게시하는 곳');
insert into board (board_name, board_type, url, secret, read_allow, write_allow, reply, modify, remove, download, upload, nAttach, aSize, display_format, board_desc)
			values ('Q&A', 'Q&A', 'qna/list.jsp?boardId=9&p=1', 'T', 'all', 'login', 'admin', 'writer', 'writer', 'login', 'login', 3, '1MB', 1, '회원과 관리자간에 질문과 답변을 주고 받는 곳');
insert into board (board_name, board_type, url, secret, read_allow, write_allow, reply, modify, remove, download, upload, nAttach, aSize, display_format, board_desc)
			values ('방명록', '방명록', 'guestbook/list.jsp?boardId=10&p=1', 'F', 'all', 'login', 'not', 'writer', 'writer', 'not', 'not', 0, '0', 3, '사이트의 방문자가 방문록을 남기는 곳');

/* 오늘의 시 등록 게시판
 *   tdate : 시를 발송하는 시간(날)
 *   send : 전송 여부 (0: 전송 안함, 1: 전송 완료)
 */
create table today_poem (
...
select * from today_poem;
delete from today_poem;
drop table today_poem;
...
select * from article;
select * from article where article_id=543;

select count(*) from ( select article_id from article where boardId = 2 and title LIKE '%가나다%') as A;

delete from article where article_id = 542;
select * from article where article_id=541;
...
select * from measure;
select * from measure where id=4159;
select * from measure where type2='누적';
select * from measure where type2='년도' and data='2016';
select count(*) from measure;
select * from measure where type2='일별' and data like '2016-12-26%';
select * from measure where id=1;
select * from measure where id=1542;
select * from download where createdAt like '2016-11-17%';
delete from measure where type2='누적';
delete from measure where data = '11';
update measure set data='2016-12' where type2='월별' and data='2016-12M';
...
select count(*) from download where createdAt like '2016-12-30%';
select count(*) from play where createdAt like '2016-12-30%';
